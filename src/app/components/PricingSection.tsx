    ...
    <section id="pricing" className="py-16 md:py-20 px-4 md:px-6 border-b border-white/10 bg-gradient-to-b from-black via-zinc-950/30 to-black">
      <div className="container mx-auto">
        <div className="mx-auto flex max-w-6xl flex-col items-center gap-4 md:gap-6 text-center">
          <div className="text-xs md:text-sm text-white/50 mb-2 md:mb-3 tracking-wide">
            PRICING
          </div>
            ...
            {pricingOptions.map((option) => (
              ...
              >
                <CardHeader className="p-4 md:p-6">
                  ...
                  <span
                    className={`font-bold ${
                      option.isRecommended ? "text-3xl md:text-4xl" : "text-2xl md:text-3xl"
                    }`}
                  >
                    {option.price}
                  </span>
                  ...
                  <ul className="space-y-2 md:space-y-3">
                    {option.features.map((feature, index) => (
                      <li key={index} className="flex items-start gap-2">
                        <Check className="size-4 md:size-5 flex-shrink-0 mt-0.5" />
                ...
                <CardFooter className="mt-auto p-4 md:p-6">
                  <button
                    className={`w-full h-11 md:h-12 px-4 md:px-6 font-medium text-sm md:text-base transition-all duration-300 relative overflow-hidden group rounded-md ${
                      option.isRecommended
                        ? "bg-gradient-to-br from-white/90 to-white/70 text-black backdrop-blur-xl border border-white/20 shadow-[0_8px_32px_0_rgba(255,255,255,0.15)] hover:shadow-[0_12px_48px_0_rgba(255,255,255,0.25)] active:scale-95"
